...
ret <- parLapply(cl = cl,
                 ...
                 fun = function(cluster_id){
                   ...
                   require(tidyverse)
                   sink(file=paste0(log_path,"nelixhauser_log__",cluster_id,".txt"))
                   print(cluster_id)
                   print(Sys.time())
                   ...
                   data <- data %>%
                     select(LopNr,indexdate,rowid,indexyear,sex,indexage,comorbidity_index_Elixhauser)
                   gc()
                   
                   data$comorbidity_index_Elixhauser[is.na(data$comorbidity_index_Elixhauser)]<-0
                   ...
                   # quantile(data$comorbidity_index_Elixhauser[data$indexage %in% 60:79],probs=seq(0,1,by=0.2))
                   
                   d <- data %>%
                     group_by(indexage,sex) %>%
                     summarize(n=n(),
                               n0=sum(comorbidity_index_Elixhauser <0),
                               n1=sum(comorbidity_index_Elixhauser %in% 0),
                               n2=sum(comorbidity_index_Elixhauser %in% 1:5),
                               n3=sum(comorbidity_index_Elixhauser %in% 6:13),
                               n4=sum(comorbidity_index_Elixhauser >=14),.groups="keep")
...
n_elixhauser <- do.call(rbind,ret) %>%
  group_by(indexage,sex) %>%
  summarize(n=sum(n),
            n0=sum(n0),
            n1=sum(n1),
...
save(n_elixhauser,file=paste0(save_path,"n_elixhauser.Rdata"))
