...
ret <- parLapply(cl = cl,
                 ...
                 fun = function(cluster_id){
                   ...
                   data <- data %>%
                     select(LopNr,indexdate,rowid,indexyear,sex,indexage,comorbidity_index_CCI10)
                   gc()
                   ...
                   d <- data %>%
                     ...
                     summarize(n=n(),
                               n0=sum(comorbidity_index_CCI10 %in% 0),
                               n1=sum(comorbidity_index_CCI10 %in% 1),
                               n2=sum(comorbidity_index_CCI10 %in% 2),
                               n3=sum(comorbidity_index_CCI10 %in% 3),
                               n4=sum(comorbidity_index_CCI10  >3),.groups="keep")
...
save(n_cci,file=paste0(save_path,"n_cci.Rdata"))
