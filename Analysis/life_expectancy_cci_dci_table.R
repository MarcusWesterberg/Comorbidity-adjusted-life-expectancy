...
ret <- parLapplyLB(cl = cl,
                   ...
                   fun = function(a){
                     ...
                     LE <- LE %>%
                       select(sex,age,CCI,dci,le.ccidci)  %>% 
                       rename(le=le.ccidci) %>%
                       mutate(dci=round(dci,dig=1)) %>%
                       group_by(sex,age,CCI,dci) %>%
                       mutate(n=n(),
                              le=mean(le)) %>%
                       slice(1) %>%
                       ungroup()
...
le.table <- le.table %>%
  arrange(sex,age,CCI,dci)

###
# Save  as table
save(le.table,file="X:\\ComorbidityBase\\Works\\2024\\2024 Assessment of comorbidity indices\\Results\\life_expectancy_cci_dci_table.Rdata")



require(mgcv)
ERL.men <- le.table %>% filter(sex == "man") %>% mutate(le=log(le))
m.men <- gam(le ~ te(age,CCI,dci),
             data=ERL.men
             ,weights=ERL.men$n,
             family="gaussian")
summary(m.men)
...
le.models <- list("men"=m.men,
                  "women"=m.women)
save(le.models,file="X:\\ComorbidityBase\\Works\\2024\\2024 Assessment of comorbidity indices\\Results\\life_expectancy_cci_dci_models.Rdata")
